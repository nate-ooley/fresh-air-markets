...
test('a date roster groups confirmed and pending vendors by category with booth totals, sorted for reading', () => {
  ...
  assert.deepEqual(day.pending.map(g => [g.category, g.booths]), [['Arts & Crafts', 1]]);
  assert.deepEqual(day.totals, { confirmedVendors: 3, confirmedBooths: 4, pendingVendors: 1, pendingBooths: 1 });
  const later = rosterForDate(vendors, '2026-10-10');
  assert.deepEqual(later.totals, { confirmedVendors: 1, confirmedBooths: 2, pendingVendors: 0, pendingBooths: 0 });
  assert.deepEqual(rosterForDate(vendors, '2026-12-05').totals, { confirmedVendors: 0, confirmedBooths: 0, pendingVendors: 0, pendingBooths: 0 });
});

test('the season overview counts booths per date against capacity and never reports negative open booths', () => {
  const season = seasonOverview(vendors, ['2026-10-03', '2026-10-10', '2026-10-17'], 4);
  assert.deepEqual(season, [
    { date: '2026-10-03', confirmedVendors: 3, confirmedBooths: 4, pendingBooths: 1, capacity: 4, openBooths: 0 },
    { date: '2026-10-10', confirmedVendors: 1, confirmedBooths: 2, pendingBooths: 0, capacity: 4, openBooths: 2 },
    { date: '2026-10-17', confirmedVendors: 1, confirmedBooths: 1, pendingBooths: 0, capacity: 4, openBooths: 3 },
  ]);
});
...
test('the spreadsheet lists confirmed rows first, quotes commas and quotes, and only accepts season dates', () => {
  ...
  assert.equal(lines[1], '2026-10-03,Confirmed (no payment due),Non-Profit Organization,Food Bank,Person,p@example.com,,1,Non-Profit Organization');
  assert.equal(lines[2], '2026-10-03,Paid,Produce,Apple Acres,Person,p@example.com,,1,Vendor');
  assert.equal(lines[4], '2026-10-03,Payment pending,Arts & Crafts,"Clay Works, ""The"" Studio",Person,p@example.com,,1,Vendor');
