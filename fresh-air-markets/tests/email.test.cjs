...
test('templates carry the essentials and escape HTML', () => {
  ...
  const book = 'https://freshairmarketsandevents.com/vendor/book#token=abc.def';
  assert.match(templates.paymentReceivedEmail({ name: 'Rosa', totalCents: 4000, bookMoreLink: book }).text, /Want more Saturdays .*vendor\/book#token=abc\.def/);
  assert.doesNotMatch(templates.paymentReceivedEmail({ name: 'Rosa', totalCents: 4000 }).text, /Want more Saturdays/);
  const withdrawn = templates.bookingWithdrawnEmail({ name: 'Rosa', dates: ['2026-10-03'], note: 'You asked <b>us</b>', bookMoreLink: book });
  assert.match(withdrawn.text, /withdrawn your booking for Saturday, October 3, 2026/);
  assert.match(withdrawn.text, /choose them here .*vendor\/book#token=abc\.def/);
  assert.match(withdrawn.html, /You asked &lt;b&gt;us&lt;\/b&gt;/);
  assert.match(templates.bookingWithdrawnEmail({ name: 'Rosa', dates: [] }).text, /reply to this email or call us/);
  assert.match(templates.applicationWithdrawnEmail({ name: 'Rosa', businessName: 'Sunrise Farms', note: 'Out this year.' }).text, /withdrawn Sunrise Farms's application .*\n\nOut this year\./);
  const received = templates.bookingRequestReceivedEmail({ name: 'Rosa', dates: ['2026-11-07', '2026-11-21'], booths: 2 });
  assert.match(received.text, /2 booths on Saturday, November 7, 2026, Saturday, November 21, 2026/);
  assert.match(received.text, /held for 48 hours/);
  const declined = templates.bookingRequestDeclinedEmail({ name: 'Rosa', dates: ['2026-11-07'], note: 'Full that week.', bookMoreLink: book });
  assert.match(declined.text, /couldn't confirm .*November 7, 2026.*\n\nFull that week\.\n\nWant more Saturdays/s);
  const staff = templates.staffBookingRequestEmail({ name: 'Rosa', businessName: 'Sunrise Farms', email: 'rosa@example.com', dates: ['2026-11-07'], booths: 1, note: 'Near the entrance please', applicationId: 'app-1', origin: 'https://freshairmarketsandevents.com' });
  assert.match(staff.subject, /Date request: Sunrise Farms \(1 Saturday\)/);
  assert.match(staff.text, /Their note: Near the entrance please/);
  assert.match(staff.text, /https:\/\/freshairmarketsandevents\.com\/applications\/app-1/);
