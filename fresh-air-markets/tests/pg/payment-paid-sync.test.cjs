...
const scope = { marketId: 'qa-payment-sync-market', seasonId: '2026-2027', locationId: 'aooAnUXF0COePorBo7wL', squareEnvironment: 'sandbox', pipelineId: 'qa-pipeline', agreementStatusFieldId: 'agreement-status-field', paymentStatusFieldId: 'payment-status-field' };
...
test('concurrent workers claim a job once and a replay after delivery performs no provider work', async () => {
  ...
  let calls = 0;
  const delivery = async job => { calls++; assert.equal(job.paymentOrderId, f.order); await new Promise(r => setTimeout(r, 20)); return paymentProof(job, scope); };
...
test('provider failure preserves paid facts and schedules sanitized retry; a later verified delivery writes receipt', async () => {
  ...
  assert.equal((await dispatchPaymentPaidSync(async job => paymentProof(job, scope), scope, { sql: second })).delivered, 1);
});
...
test('expired lease cannot mark a newer worker delivery successful', async () => {
  ...
  assert.equal(await markPaymentPaidSyncDelivered(old, paymentProof(old, scope), scope, first), false);
  assert.equal(await markPaymentPaidSyncDelivered(fresh, paymentProof(fresh, scope), scope, second), true);
});

test('paid worker rejects stage-only, wrong-field, wrong-opportunity and incomplete provider proof', async () => {
  for (const malformed of [() => undefined, proof => ({ ...proof, fields: proof.fields.slice(1) }),
    proof => ({ ...proof, fields: [proof.fields[0], { fieldId: 'foreign-payment-field', fieldValue: 'Paid' }] }),
    proof => ({ ...proof, opportunityId: 'foreign-opportunity' })]) {
    const f = await seed(); await persist(f);
    assert.equal((await dispatchPaymentPaidSync(async job => malformed(paymentProof(job, scope)), scope, { sql: first })).manual_review, 1);
    const [row] = await first`SELECT delivery_receipt, last_error_code FROM fame_payment_paid_sync_outbox WHERE payment_order_id = ${f.order}`;
    assert.deepEqual(row, { delivery_receipt: null, last_error_code: 'ghl_field_proof_invalid' });
    assert.equal((await first`SELECT state FROM fame_reservations WHERE id = ${f.reservation}`)[0].state, 'paid');
  }
});

test('legacy paid-stage receipt is preserved and revalidated once with exact Signed and Paid evidence', async () => {
  const f = await seed(); await persist(f);
  ...
  const deliver = async job => { calls++; return (expected = paymentProof(job, scope)); };
  assert.equal((await dispatchPaymentPaidSync(deliver, scope, { sql: first })).delivered, 1);
  assert.equal((await dispatchPaymentPaidSync(deliver, scope, { sql: first })).delivered, 0);
  assert.equal(calls, 1);
  const [row] = await first`SELECT delivery_receipt FROM fame_payment_paid_sync_outbox WHERE payment_order_id = ${f.order}`;
  assert.deepEqual(row.delivery_receipt, expected);
  await assert.rejects(first`UPDATE fame_payment_paid_sync_outbox SET delivery_receipt = '{}'::jsonb WHERE payment_order_id = ${f.order}`, /receipt is immutable/);
});
