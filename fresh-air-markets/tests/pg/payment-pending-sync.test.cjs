...
const { seedPaymentSync, agreementProof, paymentProof } = require('./helpers/payment-sync-fixture.cjs');
...
const scope = { marketId: 'qa-payment-pending-market', seasonId: '2026-2027', locationId: 'aooAnUXF0COePorBo7wL', squareEnvironment: 'sandbox', pipelineId: 'qa-pipeline', agreementStatusFieldId: 'agreement-status-field', paymentStatusFieldId: 'payment-status-field' };
const now = new Date();
before(async () => {
...
test('committed checkout queues once; concurrent repairs and workers deliver one exact pending stage action', async () => {
  ...
  let calls = 0; const deliver = async job => { calls++; assert.equal(job.opportunityId, `opportunity-${f.app}`); return paymentProof(job, scope, 'Ready for Payment'); };
...
test('queued agreement prerequisite defers both stages without failure-budget exhaustion; exact delivered receipt unblocks', async () => {
  ...
  await persist(f);
  for (let i = 0; i < 9; i++) {
    assert.equal((await dispatchPaymentPaidSync(async job => { calls++; return paymentProof(job, scope); }, scope, { sql: second })).deferred, 1);
    await retryPaid();
  }
  assert.equal((await first`SELECT attempts FROM fame_payment_paid_sync_outbox`)[0].attempts, 0);
  await first`UPDATE fame_agreement_stage_outbox SET status = 'delivered', delivered_at = statement_timestamp(), delivery_receipt = ${first.json(agreementProof({ locationId: scope.locationId, contactId: `contact-${f.app}`, opportunityId: `opportunity-${f.app}` }, scope))} WHERE id = ${f.stageJob}`;
  assert.equal((await dispatchPaymentPaidSync(async job => { calls++; return paymentProof(job, scope); }, scope, { sql: second })).delivered, 1);
...
test('missing or failed agreement stage proof requires review and never guesses a stage transition', async () => {
  for (const agreementStage of ['missing', 'failed']) {
    ...
    await persist(f);
    assert.equal((await dispatchPaymentPaidSync(async job => { calls++; return paymentProof(job, scope); }, scope, { sql: second })).manual_review, 1);
...
test('webhook commits while pending provider operation holds advisory lock; paid defers then advances without regression', async () => {
  ...
  const pending = await dispatchPaymentPendingSync(async job => {
    assert.equal((await persist(f)).kind, 'paid', 'webhook must not block on reservation row locks');
    const paid = await dispatchPaymentPaidSync(async job => { writes.push('confirmed'); stage = 'confirmed'; return paymentProof(job, scope); }, scope, { sql: second });
    assert.equal(paid.deferred, 1); assert.equal(writes.length, 0);
    writes.push('pending'); stage = 'pending'; return paymentProof(job, scope, 'Ready for Payment');
  }, scope, { sql: first });
  assert.equal(pending.cancelled, 1, 'paid evidence supersedes pending receipt');
  await retryPaid();
  assert.equal((await dispatchPaymentPaidSync(async job => { writes.push('confirmed'); stage = 'confirmed'; return paymentProof(job, scope); }, scope, { sql: second })).delivered, 1);
...
test('delivered agreement with the wrong configured field cannot unlock pending or paid work', async () => {
  const f = await seed(); let calls = 0;
  const wrongScope = { ...scope, agreementStatusFieldId: 'different-agreement-field' };
  assert.equal((await dispatchPaymentPendingSync(async () => { calls++; }, wrongScope, { sql: first })).manual_review, 1);
  await persist(f);
  assert.equal((await dispatchPaymentPaidSync(async () => { calls++; }, wrongScope, { sql: first })).manual_review, 1);
  assert.equal(calls, 0);
});

test('pending receipt requires Signed and exact Ready for Payment field evidence', async () => {
  const f = await seed();
  const result = await dispatchPaymentPendingSync(async job => paymentProof(job, scope, 'Pending'), scope, { sql: first });
  assert.equal(result.manual_review, 1);
  const [row] = await first`SELECT status, delivery_receipt, last_error_code FROM fame_payment_pending_sync_outbox WHERE payment_order_id = ${f.order}`;
  assert.deepEqual(row, { status: 'manual_review', delivery_receipt: null, last_error_code: 'ghl_field_proof_invalid' });
});

test('legacy pending stage success is requeued for current eligibility and cancelled after expiry without a CRM call', async () => {
  const f = await seed();
  ...
  assert.equal(row.delivery_receipt, null); assert.equal(row.legacy_delivery_receipt.status, 'delivered');
});
