...
import { dispatchPaymentPendingSync } from "@/lib/payment-pending-sync-pg";
import { deliverPaymentPendingToGhl, readPaymentPendingDeliveryConfig } from "@/lib/ghl-payment-pending-delivery";
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    const config = readPaymentPendingDeliveryConfig(process.env);
    if (!process.env.DATABASE_URL) throw new Error("Storage unavailable.");
    const result = await dispatchPaymentPendingSync(job => deliverPaymentPendingToGhl(job, config), config, { limit: 1 });
    return Response.json(result, { headers });
  } catch {
    return Response.json({ error: "Payment pending sync is unavailable." }, { status: 503, headers });
  }
}
