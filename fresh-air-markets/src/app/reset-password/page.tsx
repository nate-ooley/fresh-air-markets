...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    // Keep the single-use token out of history and referrers.
    window.history.replaceState(null, "", window.location.pathname);
    if (!token.current) setError("This reset link is incomplete. Open the link from your email again, or request a new one.");
    setReady(true);
  }, []);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/reset-password", {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token: token.current, password }),
      });
      const data = await res.json().catch(() => ({}));
      if (res.ok) { setDone(true); token.current = null; }
      else setError(data.error ?? "We could not reset your password. Please try again.");
    } catch {
    ...
    <main className="flex min-h-screen items-center justify-center bg-pine-deep px-6 py-16">
      <div className="animate-rise w-full max-w-sm">
        ...
        <form onSubmit={submit} className="rounded-3xl bg-cream p-8 shadow-2xl">
          <h1 className="font-display text-3xl text-pine-deep">Choose a new password</h1>
          {done ? <>
            <p role="status" className="mt-4 rounded-xl bg-pine/10 p-3 text-sm text-pine">Your password has been changed. Sign in with it now.</p>
            ...
            <p className="mt-1 text-sm text-ink/60">At least {MIN_LENGTH} characters. This link works once and expires 30 minutes after it was sent.</p>
            ...
            <button type="submit" disabled={busy || !ready || !token.current || !password || !confirm} className="mt-5 w-full rounded-xl bg-pine py-3 font-semibold text-cream transition enabled:hover:bg-leaf disabled:opacity-40">
              {busy ? "Saving…" : "Save new password"}
            </button>
            <p className="mt-6 text-center text-sm text-ink/60">
              <Link href="/forgot-password" className="font-semibold text-clay hover:underline">Request a new link</Link>
            </p>
          </>}
        </form>
      </div>
