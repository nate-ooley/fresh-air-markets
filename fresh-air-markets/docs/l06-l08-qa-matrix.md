...
| ID | QA scenario | Expected exact mapping and proof | Current status | Blocker to green |
| --- | --- | --- | --- | --- |
| L08-1 | Accept valid private insurance uploads | For valid PDF, PNG, and JPEG samples, a private transfer worker records each source file/event against its exact `applicationId`; scanner results advance the matching `documentId` and version only. Capture the bounded validation result, scanner event, and exact ledger/outbox IDs. | **RED** — portal validation and ledger code exist. The tested private-transfer primitive derives its own size, digest, and bounded byte samples and cleans failed objects; the HighLevel-bound ingress resolves an existing application only by exact contact, immutable opportunity, location, market, and season. User-confirmed form-validation email receipt and one reply are partial inbox evidence only; they do not prove an actual private transfer or scan. | A configured private object-store adapter, scanner, migrations, and QA deployment are absent. |
