    ...
    if (options.production) {
      console.log(JSON.stringify({ command: options.command, ...result, ready: false,
        ...
        next: result.demoAccountPresent
          ? 'Remove the public demo tenant (remove-demo-tenant --production), then apply and check migrations 001–022 with --production.'
          : 'Set FAME_MARKET_ACCOUNT_ID and FAME_BOOTH_CAPACITY in Production, then apply and check migrations 001–022 with --production.' }));
      return;
    }
    console.log(JSON.stringify({ command: options.command, ...result, ready: false,
      config: { FAME_MARKET_ACCOUNT_ID: result.accountId, FAME_SEASON_ID: '2026-2027', FAME_BOOTH_CAPACITY: options['qa-capacity'] },
      next: 'Apply and check migrations 001–022; deploy Preview and verify private manager login. QA capacity is not Production capacity.' }));
